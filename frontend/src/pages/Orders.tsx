...
import { ClipboardList, User, Phone, Hash, Truck, CheckCircle, Clock, DollarSign, XCircle, ShoppingBag, CreditCard } from 'lucide-react';
...
export const Orders: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'dine_in' | 'delivery' | 'takeout'>('all');
  ...
  const getDeliveryTypeLabel = (type: string) => {
    if (type === 'dine_in') return 'Mesa/Salão';
    if (type === 'takeout') return 'Para Viagem';
    return 'Delivery';
  };

  const getDeliveryTypeIcon = (type: string) => {
    if (type === 'dine_in') return Hash;
  ...
  return (
    <Layout>
      <div className="space-y-6">
        ...
        {/* Filtros */}
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:flex gap-2 lg:gap-3">
          <button
            onClick={() => setFilter('all')}
            className={`px-3 py-2 text-sm lg:text-base rounded-lg font-medium transition-all ${
              filter === 'all'
          ...
          >
            ...
            <span className="hidden sm:inline">Mesa/Salão</span>
            <span className="sm:hidden">Mesa</span> ({tabs.filter(t => t.deliveryType === 'dine_in').length})
          </button>
          <button
          ...
          >
            ...
            <span className="hidden sm:inline">Para Viagem</span>
            <span className="sm:hidden">P/ Viagem</span> ({tabs.filter(t => t.deliveryType === 'takeout').length})
          </button>
        </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {isClosing ? (
                      ...
                      <div className="mt-4 space-y-2">
                        <Button
                          fullWidth
                          onClick={() => navigate(`/checkout?tabId=${tab.id}`)}
