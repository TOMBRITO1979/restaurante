...
class PaymentsController {
    ...
    async createPaymentIntent(req, res) {
        try {
          ...
          `, validatedData.tabId);
                    if (tabData && tabData.length > 0 && tabData[0].orders) {
                        ...
                        const allItems = [];
                        tabData[0].orders.forEach((order) => {
        ...
        `, crypto.randomUUID(), paymentIntent.id, validatedData.tabId || null, validatedData.amount, paymentIntent.currency, stripeService_1.stripeService.convertStatus(paymentIntent.status), JSON.stringify(paymentIntent.metadata), JSON.stringify({
                id: paymentIntent.id,
    ...
    async getPaymentIntent(req, res) {
        try {
            ...
            const paymentIntent = await stripeService_1.stripeService.retrievePaymentIntent(paymentIntentId, tenantSchema);
            // Get payment record from database
            const db = (0, database_1.getTenantClient)(tenantSchema);
            const paymentRecords = await db.$queryRawUnsafe(`
    ...
    async confirmPayment(req, res) {
        try {
            ...
            const paymentIntent = await stripeService_1.stripeService.confirmPaymentIntent(validatedData.paymentIntentId, tenantSchema, validatedData.paymentMethodId);
        ...
        `, stripeService_1.stripeService.convertStatus(paymentIntent.status), paymentIntent.payment_method?.toString() || null, JSON.stringify(paymentIntent), paymentIntent.id);
            res.json({
                success: true,
                data: {
                    id: paymentIntent.id,
    ...
    async cancelPayment(req, res) {
        try {
            ...
            const paymentIntent = await stripeService_1.stripeService.cancelPaymentIntent(paymentIntentId, tenantSchema);
            // Update database record
            const db = (0, database_1.getTenantClient)(tenantSchema);
            await db.$executeRawUnsafe(`
        UPDATE "${tenantSchema}"."stripe_payments"
        ...
        `, 'canceled', JSON.stringify(paymentIntent), paymentIntent.id);
            res.json({
                success: true,
    ...
    async createRefund(req, res) {
        try {
            ...
            // Create refund
            const refund = await stripeService_1.stripeService.createRefund(paymentIntentId, tenantSchema, amountInCents, reason);
            // Update database record
            const db = (0, database_1.getTenantClient)(tenantSchema);
            await db.$executeRawUnsafe(`
        UPDATE "${tenantSchema}"."stripe_payments"
        ...
        `, JSON.stringify({ refund }), paymentIntentId);
            res.json({
                success: true,
    ...
    async completePayment(req, res) {
        try {
            ...
            const db = (0, database_1.getTenantClient)(tenantSchema);
            // Get tab details
            const tabs = await db.$queryRawUnsafe(`SELECT * FROM "${tenantSchema}"."tabs" WHERE "id" = $1 LIMIT 1`, tabId);
            if (tabs.length === 0) {
        ...
        SET "status" = 'succeeded',
            ...
            "updatedAt" = NOW()
        WHERE "paymentIntentId" = $3
        `, paymentIntent.payment_method?.toString() || null, JSON.stringify(paymentIntent), paymentIntentId);
            // Create sale record
            await db.$executeRawUnsafe(`
        ...
        `, crypto.randomUUID(), totalAmount, totalAmount, 'stripe', 'completed', tab.deliveryType || 'dine_in', tab.tableNumber || null, tab.phoneNumber || null, 0, // discountRate
            0, // discountAmount
            0, // tipRate
            0, // tipAmount
            0, // taxRate
