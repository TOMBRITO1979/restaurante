...
class ReportsController {
    ...
    async getRevenueReport(req, res) {
      ...
      `, ...params);
            ...
            res.json({
                ...
                byDeliveryType: byDeliveryType.map(item => ({
     ...
     */
    async getDeliveryTimeReport(req, res) {
        try {
            const tenantSchema = req.tenantSchema;
            const db = (0, database_1.getTenantClient)(tenantSchema);
            const { startDate, endDate } = req.query;
      ...
      `, ...params);
        ...
        FROM (
          SELECT
            ...
            END as time_range,
            CASE
              WHEN EXTRACT(EPOCH FROM (o."updatedAt" - o."createdAt")) / 60 <= 15 THEN 1
              WHEN EXTRACT(EPOCH FROM (o."updatedAt" - o."createdAt")) / 60 <= 30 THEN 2
              WHEN EXTRACT(EPOCH FROM (o."updatedAt" - o."createdAt")) / 60 <= 45 THEN 3
              WHEN EXTRACT(EPOCH FROM (o."updatedAt" - o."createdAt")) / 60 <= 60 THEN 4
              ELSE 5
            END as sort_order
          FROM "${tenantSchema}"."orders" o
          ${whereClause}
        ...
        ORDER BY sort_order
      `, ...params);
            const stats = deliveryTimeResult[0];
            res.json({
                ...
                distribution: distributionResult.map(item => ({
    ...
    async getConsolidatedReport(req, res) {
        try {
            ...
            const profitData = await this.getProfitDataInternal(db, tenantSchema, startDate, endDate);
            const revenueData = await this.getRevenueDataInternal(db, tenantSchema, startDate, endDate);
            const deliveryTimeData = await this.getDeliveryTimeDataInternal(db, tenantSchema, startDate, endDate);
            res.json({
                profit: profitData,
                revenue: revenueData,
    ...
    async getProfitDataInternal(db, tenantSchema, startDate, endDate) {
        const params = [];
        if (startDate) {
            params.push(new Date(startDate));
        }
    ...
    async getRevenueDataInternal(db, tenantSchema, startDate, endDate) {
        let whereConditions = [];
        const params = [];
        if (startDate) {
            whereConditions.push(`"closedAt" >= $${params.length + 1}`);
    ...
    async getDeliveryTimeDataInternal(db, tenantSchema, startDate, endDate) {
        let whereConditions = ['o."status" = \'delivered\''];
        const params = [];
        if (startDate) {
            whereConditions.push(`o."createdAt" >= $${params.length + 1}`);
