...
class ExpensesController {
    ...
    async list(req, res) {
      ...
      `, ...params);
            ...
            res.json((0, pagination_1.createPaginatedResponse)(converted, total, page, limit));
        }
        catch (error) {
    ...
    async create(req, res) {
        try {
            const tenantSchema = req.tenantSchema;
            const db = (0, database_1.getTenantClient)(tenantSchema);
            const { categoryId, description, amount, date, paymentMethod, supplier, isRecurring, recurringDayOfMonth, notes, } = req.body;
      ...
      `, id, categoryId, description, amount, new Date(date), paymentMethod, supplier || null, isRecurring || false, isRecurring ? recurringDayOfMonth : null, notes || null, now, now);
            const expense = await db.$queryRawUnsafe(`
        SELECT
          e.*,
          json_build_object(
    ...
    async update(req, res) {
        ...
        SET
          ...
          "updatedAt" = $10
        WHERE "id" = $11
      `, categoryId, description, amount, new Date(date), paymentMethod, supplier || null, isRecurring || false, isRecurring ? recurringDayOfMonth : null, notes || null, now, id);
            const expense = await db.$queryRawUnsafe(`
        SELECT
          e.*,
          json_build_object(
    ...
    async exportPDF(req, res) {
      ...
      `, ...params);
            ...
            for (const expense of converted) {
                ...
                x += colWidths.payment;
                doc.text(new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(expense.amount), x, y, { width: colWidths.amount });
                doc.moveDown(0.8);
            }
            // Finalizar PDF
            doc.end();
