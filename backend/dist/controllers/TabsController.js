...
class TabsController {
    ...
    async listOpen(req, res) {
        try {
            ...
            const whereConditions = [`t.status = 'open'`];
            const params = [];
            if (deliveryType) {
    ...
    async findOrCreate(req, res) {
        try {
            ...
            // Verificar se já existe comanda aberta para a mesa/delivery/takeout
            let existingTabs;
        ...
        `, tableNumber, deliveryType);
            ...
            else if ((deliveryType === 'delivery' || deliveryType === 'takeout') && customerPhone) {
                existingTabs = await db.$queryRawUnsafe(`
          SELECT * FROM "${tenantSchema}"."tabs"
      ...
      `, tabId, tableNumber || null, deliveryType || 'dine_in', customerName || null, customerPhone || null);
            const newTabs = await db.$queryRawUnsafe(`
        SELECT * FROM "${tenantSchema}"."tabs" WHERE id = $1
      `, tabId);
            res.status(201).json(newTabs[0]);
        }
        catch (error) {
    ...
    async addOrder(req, res) {
        ...
        INSERT INTO "${tenantSchema}"."orders" (
          id, "tabId", status, notes, "createdAt", "updatedAt"
        ) VALUES (
          $1, $2, 'pending', $3, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
        )
      `, orderId, tabId, notes || null);
        ...
        `, (0, uuid_1.v4)(), orderId, item.productId, item.productName, parseInt(item.quantity), parseFloat(item.unitPrice), totalPrice, item.notes || null);
            }
    ...
    async markOrderDelivered(req, res) {
        try {
    ...
    async closeTab(req, res) {
      ...
      `, saleId, tabId, tab.tableNumber || null, tab.deliveryType || 'dine_in', tab.customerName || null, tab.customerPhone || null, subtotal, discountRate, discountAmount, tipRate, tipAmount, taxRateValue, taxAmount, total, amountPaidValue, changeAmount, paymentMethod, JSON.stringify(tab.orders || []), tab.createdAt);
            // Fechar comanda
            await db.$executeRawUnsafe(`
        UPDATE "${tenantSchema}"."tabs"
        SET status = 'closed',
