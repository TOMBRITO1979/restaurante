...
exports.recurringExpensesService = exports.RecurringExpensesService = void 0;
...
class RecurringExpensesService {
        ...
        try {
            ...
            for (const company of companies) {
                try {
                    ...
                    console.error(`[RecurringExpenses] Erro ao processar empresa ${company.name} (${company.schemaName}):`, error);
     ...
     */
    async processTenantExpenses(schemaName, companyName) {
    ...
    `, currentDay);
        ...
        for (const template of recurringExpenses) {
          ...
          WHERE "recurringTemplateId" = $1
            ...
            AND "date" <= $3
          LIMIT 1
        `, template.id, startOfMonth, endOfMonth);
                if (existing.length > 0) {
                    console.log(`[RecurringExpenses] Despesa já criada este mês para template ${template.id} em ${companyName}`);
                    continue;
                }
                // Criar nova despesa baseada no template
                const newExpenseId = (0, uuid_1.v4)();
        ...
        `, newExpenseId, template.categoryId, template.description, template.amount, newDate, template.paymentMethod, template.supplier, false, // Nova despesa não é recorrente
                null, template.id, // Referência ao template original
                template.notes, now, now);
                console.log(`[RecurringExpenses] Despesa criada: ${template.description} (R$ ${template.amount}) em ${companyName}`);
            }
            catch (error) {
                console.error(`[RecurringExpenses] Erro ao criar despesa para template ${template.id} em ${companyName}:`, error);
...
exports.recurringExpensesService = new RecurringExpensesService();
//# sourceMappingURL=recurringExpensesService.js.map