...
async function migrateStripeTables() {
    try {
        ...
        const schemasResult = await database_1.prisma.$queryRawUnsafe(`
      SELECT schema_name
    ...
    `);
        ...
        `);
                // Add indexes
                await database_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "idx_stripe_payments_intent" ON "${schemaName}"."stripe_payments"("paymentIntentId")`);
                await database_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "idx_stripe_payments_tab" ON "${schemaName}"."stripe_payments"("tabId")`);
                await database_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "idx_stripe_payments_status" ON "${schemaName}"."stripe_payments"("status")`);
                await database_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "idx_stripe_payments_created" ON "${schemaName}"."stripe_payments"("createdAt")`);
...
// Run migration
migrateStripeTables();
//# sourceMappingURL=migrate-stripe-tables.js.map