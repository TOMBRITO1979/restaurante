import { getStripeClient, getStripeConfig } from '../config/stripe';
import {
...
export class StripeService {
  ...
  async createPaymentIntent(
    ...
    try {
      ...
      const stripeClient = await getStripeClient(tenantSchema);
   ...
   */
  async retrievePaymentIntent(paymentIntentId: string, tenantSchema: string): Promise<Stripe.PaymentIntent> {
    try {
      const stripeClient = await getStripeClient(tenantSchema);
   ...
   */
  async cancelPaymentIntent(paymentIntentId: string, tenantSchema: string): Promise<Stripe.PaymentIntent> {
    try {
      const stripeClient = await getStripeClient(tenantSchema);
   ...
   */
  private async findOrCreateCustomer(stripeClient: Stripe, email?: string, name?: string): Promise<Stripe.Customer> {
    try {
   ...
   */
  async getPaymentMethod(paymentMethodId: string, tenantSchema: string): Promise<Stripe.PaymentMethod> {
    try {
      const stripeClient = await getStripeClient(tenantSchema);
  ...
  convertStatus(stripeStatus: Stripe.PaymentIntent.Status): StripePaymentStatus {
    switch (stripeStatus) {
      ...
      case 'canceled':
        return StripePaymentStatus.CANCELED;
      case 'processing':
