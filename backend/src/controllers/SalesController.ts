...
export class SalesController {
  ...
  async list(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const convertedSales = (sales as any[]).map(sale => ({
        ...sale,
  ...
  async printReceipt(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const tenantSchema = req.tenantSchema!;
      const db = getTenantClient(tenantSchema);
      ...
      if (settings.city || settings.state || settings.zipCode) {
        ...
        if (settings.zipCode) locationParts.push(`CEP: ${settings.zipCode}`);
      ...
      if (converted.paymentMethod === 'stripe_card') {
        for (const item of items) {
          ...
          doc.font('Helvetica').text(
            `  ${qty} x R$ ${unitPrice.toFixed(2)} = R$ ${totalPrice.toFixed(2)}`,
            { align: 'left' }
          );
      ...
      if (converted.discountAmount > 0) {
        doc.text(
          `Desconto (${converted.discountRate.toFixed(2)}%): -R$ ${converted.discountAmount.toFixed(2)}`,
          { align: 'right' }
        );
      }
      ...
      if (converted.tipAmount > 0) {
        doc.text(
          `Gorjeta (${converted.tipRate.toFixed(2)}%): +R$ ${converted.tipAmount.toFixed(2)}`,
          { align: 'right' }
        );
      }
      ...
      if (converted.taxAmount > 0) {
        doc.text(
          `Imposto (${converted.taxRate.toFixed(2)}%): +R$ ${converted.taxAmount.toFixed(2)}`,
          { align: 'right' }
        );
      }
      ...
      doc.fontSize(8).font('Helvetica');
      doc.text(
        `Pagamento: ${paymentMethodLabels[converted.paymentMethod] || converted.paymentMethod}`,
        { align: 'center' }
      );
